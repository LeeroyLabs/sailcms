        ...
        foreach ($configs as $fieldSettings) {
            ...
            $fieldSettings->inputSettings->each(function ($index, $fieldsData) use (&$parsedConfigs) {
                ...
                $fieldsData->settings->each(function ($key, $setting) use (&$settings) {
                    $settingValue = $setting->value ?? $setting->options;
                    if (isset($setting->value)) {
                        $settingValue = EntryLayout::parseSettingValue($setting->type, $setting->value);
                    }
                    $settings->pushKeyValue($setting->name, $settingValue);
                });
                // TODO: Needs to convert to database fields data(key => value) - needs to look at fields generated by php and graphql to see difference and make it work
                // TODO: Do the same for graphql
